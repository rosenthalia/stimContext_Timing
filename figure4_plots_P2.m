...
timesUsed = [-300  -225  -150   -75     0    75   150   225   300]; % timing offsets tested for P2
...
offsetMat = zeros(9,3,2,4); %count of reported order over all sessions, times x ans x runtype x amplitude
totalAnsByTime = zeros(10,9,2); % total felt sensations, days x times x runType
StimFirstAnsByTimeByAmp = zeros(10,9,4,2); % count reported order by session, days x times x amp x runtype
VisFirstAnsByTimeByAmp = zeros(10,9,4,2); % count reported order by session, days x times x amp x runtype
sameAnsByTimeByAmp = zeros(10,9,4,2); % count reported order by session, days x times x amp x runtype
totalAnsByTimeByAmp =  zeros(10,9,4,2); % days x times x amp x runtype
...
for di = 1:size(dataStruct,1) % for each day
    for si = 1:size(dataStruct,2) % for each run
        if dataStruct{di,si}.runType ~=0
            ...
            feltTrials =~strcmp(dataStruct{di,si}.trialSenOrder,'NaN') & dataStruct{di,si}.isCatchTrials==0;
...
disp('P2 results: temporal order judgements')
...
for aI = 1:2 % for 100uA, 60ua
    ...
    for rI = 1:2
        ...
        a(1) = plot(timesUsed,perReportVisFirst(:,rI,aI),'LineWidth', 3, 'Color',timingReportColors(1,:));
        hold on
        a(2) = plot(timesUsed,perReportSame(:,rI,aI),'LineWidth', 3,'Color',timingReportColors(2,:));
        a(3) = plot(timesUsed,perReportStimFirst(:,rI,aI),'LineWidth', 3,'Color',timingReportColors(3,:));
        errorbar(timesUsed,perReportVisFirst(:,rI,aI), perVisFSem(:,1,aI),perVisFSem(:,rI,aI),'.', 'Color',timingReportColors(1,:),...
            'LineWidth',2);
        errorbar(timesUsed,perReportSame(:,rI,aI), perSameSem(:,1,aI),perSameSem(:,rI,aI),'.', 'Color',timingReportColors(2,:),...
            'LineWidth',2);
        errorbar(timesUsed,perReportStimFirst(:,rI,aI), perStimFSem(:,1,aI),perStimFSem(:,rI,aI),'.', 'Color',timingReportColors(3,:),...
            'LineWidth',2);
    ...
    sgtitle(['P2: ' num2str(ampsUsed(aI)) 'uA trials'])
end
...
if toLoad == 1
    ...
    load(['..\Data\stimContext_Timing_P2_gaussianBootByAmp.mat']);
    ...
    for cati = 1:2 % for each condition
        for ai = 1:2 % for 100uA and 60uA
            ...
            thisX = repmat(timesUsed,size(sameAnsPerByAmp,1),1); % make matrix of times tested
            ...
            pOffsetA =  gausParamsA(cati,ai,1).*exp(-((timesUsed'-gausParamsA(cati,ai,2))./gausParamsA(cati,ai,3)).^2);
...
for ai = 1:2 % for 100, 60uA trials
    ...
    title(['P2: ' ampNames{ai} 'uA, Gaussian fit to simultaneous answer rates'])
    
    % get PSS (peak of each gaussian)
    disp(['P2: ' ampNames{ai} 'uA Gaussian values'])
    disp('PSS (point of subjective simultaneity/peak of each gaussian)')
    disp(['realistic PSS = ' num2str(gausParamsA(1,ai, 2)) 's, 95%CI [' num2str(gausParamsA_CI05(1,ai, :,2)) ']'])
    disp(['abstract PSS = ' num2str(gausParamsA(2, ai,2)) 's, 95%CI [' num2str(gausParamsA_CI05(2, ai,:,2)) ']'])
