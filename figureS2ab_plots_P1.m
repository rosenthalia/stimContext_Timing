...
for sesh = 1:size(eyeData,1) % for every set collected
    ...
    % pull catch trials
    for ci = 1:4 %for every catch trial
...
disp(['P1 abstract trials usable: ' num2str(cA) '/36 = ' num2str(100*cA/36) '%'])
disp(['P1 realistic trials usable: ' num2str(cR) '/36 = ' num2str(100*cR/36) '%'])
...
for ch = 1:6 % for each channel to plot
    ...
    title(['P1: eye feature ' num2str(ch)]);
end
...
sgtitle(['P1: EYE linear regression tuned channels relative to baseline'])
